...
import { View, Text, Button, FlatList, TouchableOpacity, StyleSheet, SafeAreaView, ScrollView, Dimensions, Image } from "react-native";
 ...
 const item = route.params;
  ...
  const approveNews = (id) => {
    setNewsList((prevList) =>
      prevList.map((item) => (item.id === id ? { ...item, published: true } : item))
    );
  };
      ...
      <View style={styles.newsItemContainer}>
        ...
        <View style={styles.actionContainer}>
          <TouchableOpacity
            ...
            style={[styles.actionButton, { borderColor: item.published ? "green" : COLORS.gray }]}
          >
            <Icon name="check-circle" size={20} color="green" />
            <Text style={[styles.actionText, approveButtonTextStyle]}>Phê duyệt</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => deleteNews(item.id)}
            style={[styles.actionButton, { borderColor: item.published ? "red" : COLORS.gray }]}
          >
            <Icon name="delete" size={20} color="red" />
            <Text style={[styles.actionText, deleteButtonTextStyle]}>Xóa tin</Text>
          </TouchableOpacity>
        </View>
        <TouchableOpacity
